...
- [ ] **リアクション（kind 7）を検証ポーリングの対象に含める**
  ...
  - 影響は 2 つ。**(1)** 投稿本文と入れ子の引用カードは検証済みになるまで薄く表示され
    ✓ が付くが、リアクションのチップにはその区別が無い。**(2)** リレーが不正な kind 7 を
    キャッシュから削除しても、それはイベントとして届かないため、**チップの数字は
    リロードするまで減らない**（リロードすればストレージから消えた分は届かず正しくなる）
