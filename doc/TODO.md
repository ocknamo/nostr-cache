...
実装状況と残作業の一覧。ここがプロジェクトの**実装状況の唯一の情報源**で、README や
各パッケージの README はこのファイルを参照する。**片付いた項目は消さずに `[x]` を付けて
残す**（何をどう解決したかが次の判断材料になるため）。詳しい経緯は git の履歴と
各設計書（[doc/](.) 以下）を参照。

## 現状（2026-07）

リレーコア（イベント種別処理・検証・ストレージ・購読管理）は実装済みで、モノレポ全体の
build / typecheck / test は CI で緑。ブラウザ内ローカルリレーへのエンドツーエンド配線、
上流リレーへの透過キャッシュ化（リードスルー / ライトスルー）、server の永続化
（オプトイン。`NOSTR_DB_PATH` で `node:sqlite`）、公開デモ（GitHub Pages）、
埋め込みウィジェットも実装済み。パッケージの一覧は [README](../README.md#プロジェクト構成) を参照。

対応 NIP は **NIP-01・NIP-02**（kind 3 を replaceable として扱う範囲）**・NIP-09**
（削除リクエスト・kind 5）。適用ルールは [api.md](./api.md) を参照。

主な既知の制約（詳細は以下の各項目）:

- 削除済みイベントの「復活」は防げない（NIP-09 の残課題）
- NIP-40（`expiration` タグ）・NIP-11（リレー情報ドキュメント）・NIP-42（AUTH）は未対応
- 時間窓ベースのレート制限は未実装
...
- [ ] **上流が全滅している間の投稿を再送するキューを持つ**
  - 現状: ライトスルーは fire-and-forget で、接続中の上流が 1 台も無ければ EVENT は
    転送されずに失われる。ローカル保存は成功するのでクライアントへの `OK` は `true` で
    返り、**投稿者は失敗に気づけない**
  - 対策の方向: 転送できなかった EVENT をストレージに積み、上流の再接続時に流す。
    保持期間・件数上限と、再送分を重複排除にどう載せるかを決める必要がある
  - 設計上の位置づけは [upstream.md](./cache-relay/upstream.md) 第8節を参照
...
- [ ] **NIP-42（AUTH）の対応**
  - 現状: 未対応。上流リレーが `AUTH` を要求する場合に応答できないため、**認証が必要な
    リレーは上流に指定できない**。リレー自身がクライアントへ `AUTH` を要求する側も未実装
  - 上流側の対応には、キャッシュリレーが署名鍵を持つか、埋め込む側から署名関数を
    受け取る仕組みが要る。鍵の扱いを決めてから着手すること
...
- [ ] **タグインデックスは保存時に 100 件で切り詰められる**
  ...
  - タグ条件のあるフィルタを `indexed_tags` で引くようにした（`query.ts`）
...
- [x] `enforceLimit` が保存 1 件ごとにテーブル全件を count している
  ...
  - 解決: 退避を保存経路から外し、`EvictionSweeper`（既定 600 秒間隔・初回は 30 秒後）へ
    移した。超過を検知したときだけ `storageMaxSize` の約 90% までまとめて落とすので、
    上限に張り付いた状態でも退避パスは走りっぱなしにならない。上限は次のスイープまで
    超過しうるソフトリミットになった。超過幅が滞在時間だけで決まらないよう、
    前回から上限の約 10% が保存された時点でも 1 回走る（保存経路が触るのは
    メモリ上のカウンタだけ）
