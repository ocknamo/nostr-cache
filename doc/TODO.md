...
- [ ] **Web クライアントとローカルリレーのエンドツーエンド配線**（目的④）
  ...
  - 注: 現行の Angular 製 web-client は廃棄方針（優先度: 低の「web-client の廃棄」参照）。
    配線・デモは廃棄後に作り直すクライアントを前提とする
...
- [ ] 未実装オプションの実装（残: `cacheStrategy` の `LRU` / `LFU` のみ）
...
- [ ] TypeScript 本体の最新化（TS 6 系への引き上げ）
  - `typescript` は `^5.0.0`（現状 5.7.x）のまま。web-client の Angular 19（`@angular/compiler-cli`）が TS `>=5.5 <5.9` を要求するため、モノレポで TS 6 に上げると npm の dedupe で web-client のビルドが壊れる（検証済み）
  - **方針決定（2026-07）: 現行の Angular 製 web-client は一旦すべて廃棄する**。廃棄（`packages/web-client` の削除）が完了すれば Angular の TS バージョン制約が消えるため、Angular アップグレードを待たずに TS 6 化できる。tsconfig 移行は完了済みで、TS 側の障害は解消済み
- [ ] web-client（Angular 製 POC）の廃棄
  - 方針: 現行実装は一旦すべて捨てる（機能完成させない）。`packages/web-client` を削除し、root の `build` / `lint:check` / CI から除外する
