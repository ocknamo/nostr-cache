...
- [ ] 未実装オプションの実装（残: 保存時刻ベース TTL のみ）
  ...
  - [x] `cacheStrategy` の `LRU` / `LFU` の本実装: `DexieStorage` スキーマ v2 で `last_accessed_at`（ミリ秒）/ `access_count` とインデックス（`last_accessed_at` / `[access_count+last_accessed_at]`）を追加し、既存データは upgrade で `created_at*1000` / `0` にバックフィル。`getEvents` のヒット時に両メタデータを一括更新（アクセス追跡。失敗しても読み出しには影響させない）。`enforceLimit` は戦略ごとに退避順を切替（FIFO=`created_at` / LRU=`last_accessed_at` / LFU=参照回数→最終アクセスの複合インデックス）。挿入も1回のアクセスとみなす
