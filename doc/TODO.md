...
- [x] 統合テストのポート採番を衝突しない方式にする
  - **対応済み**: 乱数採番（`Math.floor(Math.random() * 10000) + <帯>`）を全廃した。
    ...
    「かなりの確率で落ちる」ものだった
  - `WebSocketServer.getBoundPort()`（`HealthServer` と同じ口）を追加し、
    `NostrRelayServer.getPort()` を設定値ではなく実ポートを返すよう委譲した。
    テストは `port: 0` で起動して `getPort()` / `getHealthPort()` で読み戻す
  - ヘルスチェックの既定ポートは「WebSocket ポート + 1」のままだが、WebSocket が
