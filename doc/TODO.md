...
- [ ] 未実装オプションの実装: `maxEventsPerRequest`, `storageMaxSize`, `ttl`, `cacheStrategy`(LRU/FIFO/LFU)
  - [x] 遅延バリデーション系（`validateEventsType: 'LAZY'` / `lazyValidateInterval` / `lazyValidateBatchSize`）: 保存後にバックグラウンドで定期的にバッチ検証し、不正イベントをストレージから削除。`LazyValidator` を追加し relay の connect/disconnect でタイマーを開始/停止。タイポの旧名 `lazyValidateBachSize` は後方互換の非推奨別名として受理（正: `lazyValidateBatchSize`）。現状 in-process `publishEvent()` のスコープ（トランスポート経由 EVENT の検証は従来どおり常時実行）
