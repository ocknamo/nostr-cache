...
- [ ] 未実装オプションの実装: `maxEventsPerRequest`, `ttl`, 遅延バリデーション系（`lazyValidateInterval`, `lazyValidateBachSize`）
  - 注: `lazyValidateBachSize` は既存のタイポ（正: `lazyValidateBatchSize`）。実装時にリネームすると破壊的変更になる点に留意
  - [x] `storageMaxSize` + `cacheStrategy`（FIFO）: ストレージ層（`DexieStorage`）で実装。`maxSize` 超過時に保存のたびに古いイベント（`created_at` 昇順）を退避。`new DexieStorage(name, { maxSize, cacheStrategy })` / server の `storageOptions` で設定。`LRU`/`LFU` は読み出し時アクセス追跡（スキーマ拡張）が必要なため未差別化で FIFO にフォールバック＋警告（別タスク）。なお `NostrRelayOptions` 経由の `storageMaxSize`/`cacheStrategy` は injected storage を再設定できないため未配線（ストレージ層で設定する設計）
