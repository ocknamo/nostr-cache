...
- [ ] 未実装オプションの実装: `storageMaxSize`, `cacheStrategy`(LRU/FIFO/LFU), 遅延バリデーション系（`lazyValidateInterval`, `lazyValidateBachSize`）
  ...
  - [x] `ttl` の実装: `created_at` が `now - ttl` より古い（鮮度切れの）キャッシュイベントを REQ 応答 / `subscribe()` 再生から除外。リレー読み出し層のフィルタとして実装（保存時刻ベース TTL はストレージ層の保存時刻保持が必要なため対象外）
  - [ ] 保存時刻ベース TTL（"キャッシュ投入からの経過時間"）の実装: 現行 `ttl` は `created_at`（イベント作成時刻）基準の read-side フィルタ。投入時刻基準にするには、ストレージ層（`DexieStorage`）に保存時刻（`cached_at` 等）を持たせるスキーマ拡張と、それに基づく除外/退避ロジックが必要
