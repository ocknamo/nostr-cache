...
- **`max-follows` の既定値**。**上流レイテンシは根拠にならないことが実測で確定した**
  （982 → 500 で約 10ms、§7.2-3）。「キャッシュ上の kind 1 蓄積量 × フォロー数」で
  見積もる（§7）。切り捨てを先頭から採るか末尾から採るかも同時に決める。
  なお実測対象は 882–982 人フォローで、既定 500 だと**半分近く落ちる**。
  上流が 982 人を問題なく捌く以上、上限の根拠はクライアント側だけになるので、
  ブラウザ側の実測（§7.2 の「未実施」参照）が出るまで 500 は暫定値である
