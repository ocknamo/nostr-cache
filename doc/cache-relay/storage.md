...
最終判定は `eventRowMatchesFilter` が行うので、選択が変わっても結果は変わらない。
変わるのは**答えを出すために読む行数**だけである。
...
トレードオフ: 指定したタグ値に一致する行が、併記された `kinds` に一致する行より多い
場合は不利になる（`{kinds:[30023],'#p':[有名人]}` のような形）。実装はどちらが少ないかを
問い合わせずに決めているので、これは計測ではなくウィジェットが実際に投げる形に
合わせた選択である。併せて 7 節のタグインデックス上限にも注意すること。
