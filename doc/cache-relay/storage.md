...
2. 単一文字タグ条件（`#e` / `#p` / …）→ multiEntry の `indexed_tags`。
   `distinct()` を付ける（1 行が複数の指定値に一致しうる。root と parent の両方を
   `e` タグに持つ NIP-10 リプライなど）
3. `authors` × `kinds` → `[pubkey+kind]`
4. `authors` / `kinds` / 時間範囲の各単独・組み合わせ → `pubkey` / `kind` / `created_at`
5. どれも無ければ全件走査

タグ条件を `kinds` より先に見るのが要点で、`kind` インデックスは**その kind の全行**を
...
トレードオフ: 指定したタグ値に一致する行が、併記された `kinds` / `authors` に一致する
行より多い場合は不利になる（`{kinds:[30023],'#p':[有名人]}` のような形）。実装は
どちらが少ないかを問い合わせずに決めているので、ウィジェットが実際に投げる形に
