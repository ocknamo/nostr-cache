...
フィルタごとに、読み方が 2 モードある。`planQuery` がどちらかを選ぶ。

- **順序なし**（`buildOptimizedQuery`。4.1.1）: もっとも選択的なインデックスで絞り、
  返ってきた行を並べ替える。**一致した行**を全件読む必要がある
- **順序あり**: `created_at` を**降順**に走査する。NIP-01 の `limit` が求める順序
  そのものなので、`limit` 件そろった時点でカーソルを止められる。読むのは
  **走査した行**だけ

順序ありを選ぶのは `limit` があり、`ids` もインデックス可能なタグ条件も無いフィルタ。
...
`{kinds:[1],authors:[…500],limit:50}` は時間範囲を持たないので順序なしでは
...
壊すため。`parseFilter` は必ず `limit` を入れるのでこの形も順序ありに載るが、
2 の経路に落とすと「その 1 人」を探して kind 1 を全部走査することになる。同じ
キャッシュでの実測で 128ms、同じフィルタを順序なしプラン（`[pubkey+kind]`）で
引くと 8ms。著者ごとにカーソルを開けばその人の行しか読まない。

上限を 16 本にしているのは、サブレンジのシーク自体が高くつくため（`[pubkey+kind]` に
500 本積んだ順序なしプランが 538ms かかるのがまさにそれ）。「著者ごとに開くには多すぎ、
かつ一致が疎で `limit` が早く埋まらない」帯（17 人〜数百人でキャッシュにその人たちの
投稿がほとんど無い場合）は 2 の経路が順序なしより不利になりうるが、どちらも数十 ms の
...
- limitパラメータによる件数制限（**一致するイベントのうち最新 N 件を、新しい順で**（NIP-01）。
  ...
  `limit` を持つフィルタは順序ありプラン（4.1.0）に載り、走査を N 件で打ち切る。
