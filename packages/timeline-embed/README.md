...
- **`image-proxy` を指定すると、画像の読み込み先がそのプロキシ 1 か所になります。**
  ...
  `referrerpolicy="no-referrer"` が付かず、**埋め込み先ページの URL がプロキシに送られます**
  （描画に使っている `svelte-remote-image` がこの属性を受け取らないため）。
  アバターと OGP サムネイルはプロキシ経由でも従来どおり送りません
