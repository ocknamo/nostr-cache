...
| `host.clearCache()` | 保存済みイベントを全消しする。リレーは動いたままで購読も閉じないので、**画面に出ているイベントは消えません**（利用者向けの「キャッシュを削除」ならリロードと組み合わせてください）。release 済みの handle から呼ぶと例外 |
...
## 時系列の連続性（しばらく空けてから開いたとき）

REQ はまず**キャッシュ**が答え、続いて**上流**が答えます。どちらも同じ `limit` で
打ち切るため、留守にしている間に上流の新着が `limit` 件を超えると、2 つの答えは
つながりません。素直に並べると「新着 → 留守にしていた分の時間の飛び → 前回までの
キャッシュ」になります。

そこで**上流の答えが `limit` 件に達していたとき**は、その**最古の投稿より前を出しません**。
達していれば上流はまだ下を持っているのに送っていない、つまりその下は連続を保証できない、
ということなので、飛びの下に古い投稿を並べる代わりに切ります。切った分は無限スクロール
（または `loadOlder()`）が順に読み戻し、飛びの中身も上流から埋まります。

判定は最初の答えのほか、**再接続のたび**にやり直します（rx-nostr が REQ を送り直すので、
留守にしていたページはそこで新しい答えを受け取ります）。

- 上流を設定していない（キャッシュのみの）構成では何も切りません。答えが `limit` に
  届かなかったときも切りません — それが上流の持っている全部なので、欠けようがないためです
- `limit` の無いフィルタ（`filters` 属性で指定した場合）は打ち切りを判定できないため
  対象外です。フィルタが複数あるときは**合計**で判定します（合計に達していれば、どれか 1 つは
  打ち切られている）。片方だけが打ち切られた場合は判定できず、切らない側に倒します
- 連続していた古い投稿を巻き添えで落とすことがあります（上流の答えがちょうど `limit` 件で、
  その次がキャッシュの最新だった場合）。スクロールすれば戻ります
- **再接続時は、読んでいた位置より下が消えることがあります。** 落とすのは新着だけで
  `limit` 件が埋まったときなので、それだけ溜まるほど留守にしていた場合に限られます
- 上流リレーを複数指定していて、**それぞれが重ならない投稿を返す**場合、合計が `limit` を
  超えて「打ち切られた」と読み違えることがあります（多くのリレーは内容が重なるので、
  重複排除後の件数は 1 リレー分に近くなります）
- 上流の EOSE が既定 3 秒で打ち切られるほど遅いと、その時点までの答えで判定するため
  効かないことがあります。次の再接続でやり直します
- 1 つのページに**複数のウィジェット**を置くと、上流由来かどうかの判定は共有のメトリクスを
  見るため揺れることがあります（判定を分けたい場合は iframe で隔離してください）
...
| 最後に退避されるもの | プロフィール（kind 0）・フォローリスト（kind 3）・削除リクエスト（kind 5）。**退避は必ず目標件数まで落とす**ので、これらだけになったら通常の順序で退避します |
| 実行されるタイミング | 10 分ごと（初回は起動の 30 秒後）、および前回から上限の約 1 割が保存された時点。**上限を超えていたときだけ**、上限の約 90% までまとめて落とします。保存のたびに退避すると、上限に張り付いたキャッシュでは 1 件ごとに読み出しと同じテーブルのロックが挟まり、フォローリストや投稿の取得が待たされるためです |
...
- **kind 3 が退避されると毎回上流へ戻ります。** ウィジェットは kind 0 / 3 を
  優先イベント（最後に退避）として起動するので通常は起きませんが、`max-events` の
  9 割をフォロー数が超えると優先イベントだけで退避先の件数に達し、退避されます
  （[キャッシュ量の上限](#キャッシュ量の上限)）
...
- **リレーコア由来の制約がそのまま効きます**（上流 AUTH（NIP-42）未対応、NIP-40 の
  期限切れ未対応、削除済みイベントの「復活」を防げない、上流全滅中の投稿は失われる、など）。
  一覧は [doc/TODO.md](../../doc/TODO.md) を参照してください。
