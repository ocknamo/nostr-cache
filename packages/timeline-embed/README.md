...
- **プロフィールは購読を 1 本増やします**。まだ取得できていない著者をまとめた
  `{"kinds":[0],"authors":[...]}` を 200ms のデバウンスで張り直す方式で、著者は最大 200 名まで。
  超過分は短縮 pubkey のままになります（上限は「同時に問い合わせ中の人数」なので、解決した分だけ
  枠は空きます）。取得済みの著者は次回以降のフィルタから外れるため、
  **表示中に kind 0 が更新されても反映されません**（リロードで反映されます）。これは、著者が
  1 人増えるたびに取得済みの分まで上流へ問い合わせ直すのを避けるための割り切りです
