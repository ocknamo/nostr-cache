...
`kinds` に `7` を含めた場合（通知タイムラインなど）、**リアクション（NIP-25）の本文は
字形に置き換えて表示します** — `+` と空文字は ⭐、`-` は 👎。どちらも NIP-25 が
「いいね」「よくないね」として予約している綴りで、著者が書いた文字ではないためです。
それ以外の絵文字や `:shortcode:` は書かれたまま出ます（投稿詳細のチップと同じ規則。
チップと違い、`:shortcode:` の画像展開は行いません）。
