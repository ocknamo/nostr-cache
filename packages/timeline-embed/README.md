...
- **ルートから辿って繋がったものだけ**を木に入れます。リレーの `#e` は任意位置の `e` タグに
  マッチするので、同じスレッドの**別の枝**の返信も届きますが、これがそのままフィルタになります
...
`::part(replies-note)` / `::part(ref-nav)` を公開しています。`reaction-chip-<key>` の
...
- **返信の取得は深さ `replies-depth`（既定 3）まで、1 階層 `replies-limit`（既定 100）件まで、
  1 投稿につき 500 件までです。** 切られた先は「さらに N 件の返信があります」と出ます
...
- **返信は誰でも書けます。** スパム・荒らしのフィルタはありません（NIP-10 で親が繋がるか
  どうかしか見ていません）。止めたい場合は `show-replies="false"` を指定してください
