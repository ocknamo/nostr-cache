...
> **旧 `ogp-endpoint` からの移行**: JSON を返す自前エンドポイントを指定する
> `ogp-endpoint` は廃止しました。corsproxy.io を使うなら `ogp-proxy` に置き換えるだけです。
> 自前で運用を続ける場合は、エンドポイントを「`url` パラメータのページを取得して
> **HTML をそのまま返す** CORS プロキシ」に変更したうえで `ogp-proxy` に指定してください
> （投稿者が書いた任意の URL を取りに行かせる以上、**SSRF 対策とレート制限はそちら側の
> 責任**です）。
