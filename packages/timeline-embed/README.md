...
- **プロフィールは購読を 1 本増やします**。表示中のイベントの著者をまとめた
  `{"kinds":[0],"authors":[...]}` を 200ms のデバウンスで張り直します。取得済みの著者も
  含めて素直に問い合わせ直し、**上流への転送を省くかどうかはリレーが判断します**
  （`upstreamFreshness` の kind 0 の窓。既定 300 秒。JS から `acquireRelayHost` を使う場合のみ
  `profileFreshness` で変更でき、属性やクエリパラメータでは変えられません）。
  「キャッシュがまだ新鮮か」はキャッシュ自身の責務なので、ウィジェット側で二重に持たない方針です
- **著者リストは表示中のイベントから毎回作り直します**。タイムラインの保持上限
  （既定 500 件）を超えて押し出されたイベントの著者はフィルタから外れます。pubkey は
  上流が寄越した値なので、溜め込むと「こちらが送る REQ の大きさを上流が決められる」
  ことになるためです
- **プロフィール購読を張り直すのは新しい著者が現れたときだけ**です。したがって
  **画面を開いたまま kind 0 が更新されても反映されません**（リロードで反映されます）
