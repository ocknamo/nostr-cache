...
| `note-action-label` | その押下の文言（アクセシブル名にもなる）。`note-action` が無いときは効きません | `投稿を開く` |
...
| `note-action-label` | その押下の文言（アクセシブル名にもなる） | `投稿を開く` |
...
カードと同じ**白系の背景**（枠線と影で浮かせています）で、時刻の上に開きますが、
...
- 押下対象は**カードの枠いっぱい**です（枠に重ねた透明なボタン 1 つ）。カードには何も
  足さず、ホバーで薄く色が付くだけです（`--nt-quote-hover-bg`）。`note-action-label` は
  その `title` になり、アクセシブル名は `投稿を開く: たけし` のように引用先の著者名が
  続きます（1 つの本文に引用が複数あるとき、同じ名前の押下対象が並ばないようにするため）。
