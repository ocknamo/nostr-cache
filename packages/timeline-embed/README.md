...
画面に並べるイベント数の上限です。既定 500 件で、`limit="50"` なら約 10 ページ分
遡れる計算になります。到達すると**追加読み込みを止めます** — 上限は古い方から切り捨てる
ので、止めなければ「読んでは捨てる」を繰り返すだけになるためです。`0` で上限なしに
できますが、DOM とメモリは際限なく増えます。
...
- **1 ページごとに必ず上流リレーに問い合わせます。** `until` の付いたフィルタは
  ...
  遡るほど上流への REQ が増えることになるので、`limit` を極端に小さくしないでください
