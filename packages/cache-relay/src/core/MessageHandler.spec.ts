...
describe('MessageHandler', () => {
  ...
  describe('handleMessage', () => {
    it('should handle invalid message format', () => {
      ...
      expect(responseCallback).toHaveBeenCalledWith('client1', ['NOTICE', 'Invalid message format']);
    });
    
    it('should handle unknown message type', () => {
      messageHandler.handleMessage('client1', ['UNKNOWN']);
      
      expect(responseCallback).toHaveBeenCalledWith('client1', ['NOTICE', 'Unknown message type: UNKNOWN']);
    });
  });
  ...
  describe('handleEventMessage', () => {
    it('should handle invalid EVENT message format', () => {
      ...
      expect(responseCallback).toHaveBeenCalledWith('client1', ['NOTICE', 'Invalid EVENT message format']);
    });
    ...
    it('should send OK message with error if event is invalid', () => {
      ...
      expect(responseCallback).toHaveBeenCalledWith('client1', ['OK', sampleEvent.id, false, 'invalid: event validation failed']);
    });
  });
  ...
  describe('handleReqMessage', () => {
    it('should handle invalid REQ message format', () => {
      ...
      expect(responseCallback).toHaveBeenCalledWith('client1', ['NOTICE', 'Invalid REQ message format']);
    });
  ...
  describe('handleCloseMessage', () => {
    it('should handle invalid CLOSE message format', () => {
      ...
      expect(responseCallback).toHaveBeenCalledWith('client1', ['NOTICE', 'Invalid CLOSE message format']);
    });
